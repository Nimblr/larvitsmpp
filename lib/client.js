 ...
 * @param {object} [handleCmd] - map of smpp commands
 * @return {object} (returnObj)
 */
function clientSession(sock, options, handleCmd) {
	...
	// Merge commands before login
    returnObj.handleCmd = merge(returnObj.handleCmd, handleCmd || {});
	...
	returnObj.on('incomingPduObj', function(pduObj) {
		...
		if (typeof returnObj.handleCmd[pduObj.cmdName] === 'function') {
			...
			returnObj.handleCmd[pduObj.cmdName](pduObj, returnObj);
		} else {
 ...
 * Setup a client.
 *
 * @param {object} options - host, port, username, password, tls, enqLinkTiming, errorListener
 * @param {object} handleCmd - map of functions to handle smpp commands
 * @param {function} callback(err, session)
 */
function client(options, handleCmd, callback) {
	var sock;
	...
	if (options.errorListener && typeof options.errorListener === 'function') {
		sock.on('error', options.errorListener);
		delete options.errorListener;
	}

	sock.connect(options, function() {
		var session = clientSession(sock, options, handleCmd);
