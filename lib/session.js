...
function session(sock) {
	...
	// Add a error handler by default
    returnObj.on('error', function(data) {
		var message = data && data.message || 'No info';
        log.warn('larvitsmpp: lib/session.js: session() - sock.on(error) - Socket error detected!: ' +  message);
    });
